#!/usr/bin/env node
/**
 * WF-0116/WF-0117 checks for CompozyOS selection and Graphify detection.
...
try {
  ...
  check('CompozyOS marker selects the active executor', receipt.compozy.status === 'configured');
  ...
  check('non-mutating selection leaves fixture metadata unchanged', JSON.stringify(beforeSnapshot) === JSON.stringify(afterSnapshot));
} finally {
...
try {
  ...
  check('installer reports active CompozyOS execution', installOutput.includes('CompozyOS detected: active executor priority enabled'));
  ...
  check('installed host contract routes configured CompozyOS through execute',
    readFileSync(join(installRoot, 'AGENTS.contextdevkit.md'), 'utf-8').includes('compozy-execution-priority')
    && readFileSync(join(installRoot, 'CLAUDE.md'), 'utf-8').includes('compozy-execution-priority')
    && readFileSync(join(installRoot, 'INSTRUCTIONS.md'), 'utf-8').includes('compozy-execution-priority'));
  ...
  check('doctor reports CompozyOS active execution authority', doctorOutput.includes('CompozyOS configured as the active priority executor')
    && doctorOutput.includes('auto-starts the daemon'));
