...
```
[soak] → Stop hook counts registered sessions ≥ proposeAfterSessions → nudge suggests the `distill-sessions` skill
USER runs the `distill-sessions` skill
...
USER runs the `distill-apply` skill   (or deletes the proposal)
