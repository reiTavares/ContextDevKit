...
> Draft a [Unreleased] CHANGELOG skeleton from Conventional Commits since the last tag. Drafts only — never writes the file.
> Argument: [--since <tag>]
# 📝 Draft the changelog
...
1. Run the drafter:
   ```
   node contextkit/tools/scripts/draft-changelog.mjs <user-specified argument>
   ```
