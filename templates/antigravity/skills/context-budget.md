...
> Context budget — guidance on WHICH context files to load per task type (always / on-demand / skip) to keep token cost low. Read-only advice. (ADR-0066)
> Argument: [bugfix | feature | refactor | docs | review]
# 🎯 Context Budget
...
On **Claude Code**, an `@`-import pulls a file into context on demand and is the
...
Claude Code resolves these relative to the file that contains them (e.g.
`CLAUDE.md` at the project root), so the paths above point at the installed kit
...
`CLAUDE.md` (root constitution), `contextkit/best-practices.md`,
...
| Tier | Load |
| --- | --- |
| **ALWAYS** | `CLAUDE.md`; the **specific file(s)** under suspicion; the failing test or repro. |
...
| Tier | Load |
| --- | --- |
| **ALWAYS** | `CLAUDE.md`; `contextkit/memory/project-map/00-index.md` (orient before adding); any ADR that constrains the feature's area. |
...
| Tier | Load |
| --- | --- |
| **ALWAYS** | `CLAUDE.md` (the file-size + SRP + layering rules drive the refactor); `contextkit/memory/project-map/00-index.md` (blast radius); the file(s) being refactored. |
...
| Tier | Load |
| --- | --- |
| **ALWAYS** | the doc file(s) being edited; `CLAUDE.md` §Language policy + §Documentation. |
...
| Tier | Load |
| --- | --- |
| **ALWAYS** | the **diff** under review; `CLAUDE.md` (the constitution is the rubric). |
...
3. Pull **ON-DEMAND** items with an `@`-import (Claude Code) or by opening the
