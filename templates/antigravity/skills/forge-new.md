...
> Forge a new portable Agent Package — interviews the dev (architect), routes to a provider (router), renders per-provider files (prompt-engineer + tool-designer), and packages the APF v1 under agent-packages/<name>@<semver>/. (agent-forge squad)
> Argument: <agent-name>
# 🛠️ Mode: agent-forge — new package

You just entered the `agent-forge` flow. The objective: forge a portable Agent
Package for **<user-specified argument>** (or for the name the developer gives during the interview).
