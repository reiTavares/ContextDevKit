...
> Manual premium tier (ADR-0052) — run ONE task on Claude Fable 5, the deliberately expensive/limited model. Explicit-only; never automatic.
> Argument: <the task to run on Fable>
# 🎩 the `fable` skill — the manual premium tier (ADR-0052, Phase 2)

Run **`<user-specified argument>`** on **Claude Fable 5** — the premium model the automatic tier
...
1. **Acknowledge the cost, once.** State plainly: _"Running this on Fable 5
   ...
   `<user-specified argument>`; do not pad it with extra work to "make the most of" Fable.
2. **Dispatch to a Fable subagent.** Delegate `<user-specified argument>` via the Agent tool with
