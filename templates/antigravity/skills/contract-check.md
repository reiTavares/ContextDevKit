...
- **First time / after an intentional breaking change:** save the baseline
  `node vibekit/tools/scripts/contract-scan.mjs --save` (commit
  `vibekit/memory/contract-baseline.json`).
- **Normally:** `node vibekit/tools/scripts/contract-scan.mjs` — flags any
...
The contract is whatever you declare in `vibekit/config.json` → `l5.contractGlobs`
(e.g. `["packages/shared/", "src/api/"]`). If it's empty, set it first via
`/vibe-config` — there's nothing to track until you do.

Run the right variant based on `<user-specified argument>`, show the result, and if there's
