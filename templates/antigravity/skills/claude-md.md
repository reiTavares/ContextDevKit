...
> Ensure every app/module has its own scoped CLAUDE.md, then fill each with real local rules.
> Argument: [find | scaffold]
# 📁 Modular CLAUDE.md
...
Run on `<user-specified argument>` (default: do `find`, then offer `scaffold` + fill). For a
