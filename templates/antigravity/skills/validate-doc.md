...
> Quality gate for our own planning artifacts (ADRs / roadmap) — measurability, trade-offs, no placeholders. Advisory, never blocks.
> Argument: <path/to/doc.md> [--adr | --roadmap]
# 📋 Validate a planning document

Run the document-quality rubric against **<user-specified argument>** [ADR-0030]. This is the
...
1. Run the checker:
   ```
   node contextkit/tools/scripts/validate-doc.mjs <user-specified argument>
   ```
