# Skill: forge-fallback-test

> Chaos-test the fallback path for one forged Agent Package by simulating a primary 503 on the first call. Verifies the eval scaffold survives upstream failures. (agent-forge squad)
> Argument: <agent>[@<version>] [--json]
# 🛠️ Mode: agent-forge — fallback test

Run `node contextkit/squads/agent-forge/cli/forge-eval-cli.mjs fallback-test <user-specified argument>`.
