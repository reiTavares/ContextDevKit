 ...
 *
 * Order:
 *   1. `vibekit/config.json` → `providers.review` (if set, win).
 ...
 * @param {string} [opts.configPath]  override path to vibekit/config.json
...
export async function resolveAdapter({ cwd = process.cwd(), configPath } = {}) {
  ...
  if (fromConfig) {
    ...
    if (!hit) {
      throw new ProviderError(
        ...
        `vibekit/config.json names review provider "${fromConfig}" but no matching adapter is installed`,
      );
    }
  ...
  if (!originUrl) {
    throw new ProviderError(
      'NO_ORIGIN',
      'no `origin` remote configured — cannot auto-detect a review provider. Set `providers.review` in vibekit/config.json.',
    );
  }
  ...
  if (!detected) {
    throw new ProviderError(
      'NO_MATCHING_ADAPTER',
      `no review adapter matched origin "${originUrl}". Set \`providers.review\` in vibekit/config.json or add an adapter to vibekit/runtime/providers/review/.`,
    );
  }
