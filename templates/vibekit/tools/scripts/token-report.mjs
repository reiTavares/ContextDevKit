 ...
 *
 * Usage:
 *   node vibekit/tools/scripts/token-report.mjs            # this project (table)
 *   node vibekit/tools/scripts/token-report.mjs --json
 *   node vibekit/tools/scripts/token-report.mjs --all      # every project, not just cwd
 *   node vibekit/tools/scripts/token-report.mjs --from <dir>   # read transcripts from <dir>
 */
...
function main() {
  ...
  console.log('\n🪙  VibeDevKit token report' + (flag('--all') ? ' (all projects)' : ` — ${ROOT}`) + '\n');
  if (sessions === 0) {
  ...
  if (budget.budgetPerSession > 0) {
    ...
    console.log('\nNo per-session budget set. Configure with: /vibe-config set tokens.budgetPerSession <n>');
  }
}
