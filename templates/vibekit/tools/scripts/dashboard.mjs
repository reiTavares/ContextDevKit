 ...
 *   node vibekit/tools/scripts/dashboard.mjs --watch          # live server on :4242
 *   node vibekit/tools/scripts/dashboard.mjs --watch --port=N # override port
 *   node vibekit/tools/scripts/dashboard.mjs --out=path.html  # custom snapshot output
 *
...
Live mode (--watch):
  ...
  $VIBEDEVKIT_DASHBOARD_PORT). The page reconnects automatically via SSE
  and re-renders when files in the platform dir change.

Examples:
  node vibekit/tools/scripts/dashboard.mjs
  node vibekit/tools/scripts/dashboard.mjs --watch
...
async function runLive(root) {
  ...
  process.stdout.write(`   Watching ${root}/vibekit/ — Ctrl+C to stop.\n`);
  const shutdown = () => {
