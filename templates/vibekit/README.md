...
[VibeDevKit](https://github.com/reiTavares/VibeDevKit). It is a **bounded
...
6. **Autonomy & Insight** — `/ship`, `/retro`, `/vibe-stats`, agent-forge squad.
...
- **Setup credentials for `/media-gen`** — copy `.env.example` to `.env`, fill
  in `GOOGLE_AI_API_KEY`, run with `node --env-file=vibekit/.env ...`.
