...
O VibeDevKit transforma "vibe coding" em **engenharia**: em vez de torcer pra
...
2. Copia `vibekit/.env.example` pra `vibekit/.env`, cola em `GOOGLE_AI_API_KEY=`
3. (Opcional) `VIBEDEVKIT_MEDIA_MAX_USD=5.00` pra capar custo por processo
4. Roda com `node --env-file=vibekit/.env vibekit/tools/scripts/media-gen.mjs ...`
...
Docs completos (inglês): `vibekit/README.md` e a pasta `docs/` do kit
