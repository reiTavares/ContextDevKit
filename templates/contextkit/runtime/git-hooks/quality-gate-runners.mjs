/**
 * quality-gate-runners — stack detection + the per-language gate matrix.
 *
 * The execution half of the multi-language pre-push gate (ADR-0062). `quality-gates.mjs`
 * owns the warn/block ORCHESTRATION (config, level thresholds, push-range, summary);
 * this module owns DETECTION (which stack) and DISPATCH (which lint/typecheck/test to
 * run). The two were one file that breached the line budget; the seam is real — the
 * runners take a shared accumulator as a parameter (they are not closures bound to
 * the orchestrator), so they hoist cleanly. Zero third-party deps (`node:*` only).
 *
 * COHESION NOTE: the 10-stack `runOtherGates` table is one indivisible
 * detection→dispatch unit — splitting it per-stack would scatter a single cohesive
 * table and add import noise for no second consumer.
 */
...
export function makeAccumulator(disabled) {
...
export function runNodeGates(acc, sha) {
...
export function runOtherGates(type, acc) {
  const g = acc;
  const table = {
    python() {
