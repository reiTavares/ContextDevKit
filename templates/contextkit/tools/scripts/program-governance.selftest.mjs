...
let failures = 0;
const ok = (cond, label) => { if (cond) { console.log(`  ✓ ${label}`); } else { failures += 1; console.error(`  ✗ ${label}`); } };
...
ok(pass.ok === true, 'all-pass fixture → ok:true');
ok(pass.checks.adrSchemaV2.ok, 'all-pass → adrSchemaV2 ok');
ok(pass.checks.decisionRefs.length === 2 && pass.checks.decisionRefs.every((r) => r.ok), 'all-pass → both decisionRefs resolve');
ok(pass.checks.provenance.ok, 'all-pass → provenance ok');
ok(pass.checks.noDuplicates.ok, 'all-pass → no duplicates');
ok(pass.checks.registriesIdempotent.ok, 'all-pass → registries idempotent');

// 2. Bad schema (v1) → fail.
const badSchema = assessProgramGovernance({ ...BASE, adrRecord: { ...VALID_ADR, schemaVersion: 1 } });
ok(badSchema.ok === false && !badSchema.checks.adrSchemaV2.ok, 'schemaVersion 1 → ok:false (schema fails)');

// 3. Unresolved decision reference → fail.
const missingRef = assessProgramGovernance({ ...BASE, registry: { decisions: [] } });
ok(missingRef.ok === false && missingRef.checks.decisionRefs.some((r) => r.missing.length > 0), 'unresolved ref → ok:false (missing reported)');
...
ok(noProv.ok === false && !noProv.checks.provenance.ok, 'no approvalSource → ok:false (provenance fails)');
...
ok(dup.ok === false && !dup.checks.noDuplicates.ok, 'id-duplicate finding → ok:false (noDuplicates fails)');

// 6. Non-idempotent registry rebuild → fail.
const notIdem = assessProgramGovernance({ ...BASE, registriesIdempotent: false });
ok(notIdem.ok === false && !notIdem.checks.registriesIdempotent.ok, 'non-idempotent rebuild → ok:false');

// 7. Empty workflow list is NOT a pass (refsOk requires ≥1 plan).
const noPlans = assessProgramGovernance({ ...BASE, workflowPlans: [] });
ok(noPlans.ok === false, 'no workflow plans → ok:false (vacuous pass refused)');
