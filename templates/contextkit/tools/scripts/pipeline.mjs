...
import { move, qaReject } from './pipeline-transitions.mjs';
...
if (cmd === 'add') add({ ROOT, PIPE, sync, BANDS, SEVMAP, SLADAYS });
  ...
  console.error('Usage: pipeline.mjs <add|ingest|prioritize|wsjf|bugs|move|start|stop|validate|sync|list|qa-reject>');
  process.exit(1);
}
