 ...
 *   [a] matchScore ≥ 80 → LINK → COVERED_BY_ACCEPTED (or LEGACY_GRANDFATHERED)
 *       (§29 TABLE 5 governingThreshold=80)
 *   [b] matchScore 60..79 → SURFACE band → NEEDS_DECISION + candidate surfaced
 *       (§29 TABLE 5 confirmThreshold=60)
 *   [c] matchScore < 60 → RECOMMEND-new → NEEDS_DECISION + linkTarget null
...
// ADR-1002: legacy,   exact triple WORK_TRIPLE_A (no acceptedAndCurrent bonus)
// ADR-1003: superseded
// ADR-1004: proposed
// ADR-1005: rejected
// ADR-1006: accepted/new, platform context only (sameAreaCapability miss)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// [a] matchScore ≥ 80 → LINK → COVERED_BY_ACCEPTED
// ADR-1000: samePrimaryContext(35) + sameDecisionKind(15) + sameContractInvariant(20)
//           + acceptedAndCurrent(5) + termOverlap(≈8) = 83 ≥ 80
// ---------------------------------------------------------------------------
process.stdout.write('\n[a] matchScore ≥ 80 → LINK → COVERED_BY_ACCEPTED / LEGACY_GRANDFATHERED\n');
{
  ...
  assert('[a] matchScore ≥ 80', r.matchScore >= 80, `got ${r.matchScore}`);
  ...
  // Legacy-only: samePrimaryContext(35)+sameDecisionKind(15)+sameContractInvariant(20)+termOverlap(≈8) = 78
  // 60 ≤ 78 < 80 → SURFACE (NEEDS_DECISION, not auto-linked)
  ...
  if (lr.matchScore >= 80) {
    assert('[a] legacy-only ≥ 80 → LEGACY_GRANDFATHERED', lr.coverageMode === 'LEGACY_GRANDFATHERED', `got ${lr.coverageMode}`);
  } else {
    assert('[a] legacy-only < 80 → not COVERED_BY_ACCEPTED', lr.coverageMode !== 'COVERED_BY_ACCEPTED', `got ${lr.coverageMode}`);
  }
}

// ---------------------------------------------------------------------------
// [b] matchScore 60..79 → SURFACE band → NEEDS_DECISION; candidate surfaced
// ADR-2000: sameAreaCapability(20)+sameDecisionKind(15)+sameContractInvariant(20)
//           +acceptedAndCurrent(5)+termOverlap(≈8) = 68 → in [60,79]
// ---------------------------------------------------------------------------
process.stdout.write('\n[b] matchScore 60..79 → SURFACE band → NEEDS_DECISION + surfaced\n');
{
  ...
  if (r.matchScore >= 60 && r.matchScore <= 79) {
    assert('[b] matchScore in [60..79]', true);
    ...
    assert('[b] no auto-link when score < 80', r.coverageMode !== 'COVERED_BY_ACCEPTED', `got ${r.coverageMode}`);
  }
}

// ---------------------------------------------------------------------------
// [c] matchScore < 60 → RECOMMEND-new → NEEDS_DECISION; linkTarget null
// ADR-1006 (platform ARCHITECTURE) vs business/COMPLIANCE triple
//   sameAreaCapability=0 (different type), sameDecisionKind=0, incompatibleScope=-25,
//   incompatibleContext=-20, acceptedAndCurrent=5 → total clamped to 0
// ---------------------------------------------------------------------------
process.stdout.write('\n[c] matchScore < 60 → NEEDS_DECISION + linkTarget null\n');
{
  ...
  assert('[c] matchScore < 60', r.matchScore < 60, `got ${r.matchScore}`);
