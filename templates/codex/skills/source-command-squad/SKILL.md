...
behaviour). Agents live in `.codex/agents/` and install at **Level 4**.
...
members, and when to use each. If `.codex/agents/` isn't present, note the project
...
reference behind the lean `.codex/agents/<agent>.md`.
...
agents from `.codex/agents/_TEMPLATE.md` (sharp `description`s). Keep the
