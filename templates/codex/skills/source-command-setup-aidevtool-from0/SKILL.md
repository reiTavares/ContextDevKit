---
name: "source-command-setup-aidevtool-from0"
...
# source-command-setup-aidevtool-from0

Use this skill when the user asks to run the migrated source command `aidevtool-from0`.
...
will enforce later (`/setupcontextdevkit`, scoped AGENTS.md, `/contract-check`):
...
conventions verbatim into the project's `AGENTS.md` "Stack" block.
...
each its **own scoped AGENTS.md** with `/claude-md` — the root one is the
