...
1. **Identify what the change touches.** List the concrete files/dirs you expect to modify. Cross-
   reference `vibekit/config.json` → `l5.highRiskPaths` to see which are gated.
...
3. **Write the report** to `vibekit/memory/predictions/<YYYY-MM-DD>-<slug>.md` with: objective,
...
4. **Authorize the edits** by recording the simulation on the ledger (this unblocks the L5 gate):
   ```
   node vibekit/tools/scripts/mark-simulation.mjs "$ARGUMENTS" <covered-path-or-dir/> [more ...]
   ```
...
`node vibekit/tools/scripts/mark-simulation.mjs "BYPASS: <reason>" <path>`.
