---
description: Fleet mode — one control plane over many VibeDevKit repos (portfolio stats, cross-repo audit, CLAUDE.md rule-drift).
argument-hint: [list | add <path> | remove <path> | stats | audit | propagate <rule-file>]
---

# 🛰️ Fleet (control plane over many repos)

Operate on a **portfolio** of VibeDevKit repos at once. The registry lives outside
any repo — `~/.vibedevkit/fleet.json` (override `VIBE_FLEET_FILE`). Helper:
`vibekit/tools/scripts/fleet.mjs`.

Act on **$ARGUMENTS**:

## list / add / remove
```
node vibekit/tools/scripts/fleet.mjs add <path-to-repo>
node vibekit/tools/scripts/fleet.mjs list
```
Register the repos you want the fleet to cover (absolute paths are stored).

## stats — portfolio health
```
node vibekit/tools/scripts/fleet.mjs stats          # or --json
```
...
```
node vibekit/tools/scripts/fleet.mjs audit          # or --json
```
...
```
node vibekit/tools/scripts/fleet.mjs propagate <rule-file>
```
